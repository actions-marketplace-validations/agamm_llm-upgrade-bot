...
function formatMatch(match) {
  const location = import_picocolors.default.cyan(`${match.file}:${String(match.line)}`);
  const model = import_picocolors.default.yellow(`"${match.matchedText}"`);
  const lines = [`${location}  ${model}`];
  if (match.safeUpgrade) {
    lines.push(`  ${import_picocolors.default.green("\u2192")} safe:  ${import_picocolors.default.green(match.safeUpgrade)}`);
  }
  if (match.majorUpgrade) {
    lines.push(
      `  ${import_picocolors.default.magenta("\u2192")} major: ${import_picocolors.default.magenta(match.majorUpgrade)}`
    );
  }
  return lines.join("\n");
}
...
function formatFixReport(result, edits) {
  ...
  const lines = [];
  lines.push(import_picocolors.default.bold(`Fixed ${String(result.applied)} ${modelWord}:`));
  for (const edit of edits) {
    const location = import_picocolors.default.cyan(`${edit.file}:${String(edit.line)}`);
    const old = import_picocolors.default.red(`"${edit.oldText}"`);
    const arrow = "\u2192";
    const replacement = import_picocolors.default.green(`"${edit.newText}"`);
    const tier = import_picocolors.default.dim(`(${edit.tier})`);
